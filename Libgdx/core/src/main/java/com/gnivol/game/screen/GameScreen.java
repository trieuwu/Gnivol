...
public class GameScreen extends BaseScreen {
    ...
    public void show() {
        ...
        debugManager = new com.gnivol.game.system.debug.DebugRenderer(this);
        ...
        inputHandler.addProcessor(new InputAdapter() {
            ...
            public boolean touchDown(int screenX, int screenY, int pointer, int button) {
                if (debugManager.isDebugMode() && button == Input.Buttons.LEFT) {
                    debugManager.handleDebugClick(screenX, screenY, camera, viewport);
                    return true;
                }
                ...
                if (overlayManager.isActive()) {
                    ...
                    // Logic lấy khúc xương từ tủ lạnh
                    if ("fridge".equals(overlayManager.getSourceId()) && !game.getInventoryManager().hasItem("bone")) {
                        Vector2 relClick = overlayManager.getRelativeClick(touch.x, touch.y);
                        if (relClick != null && relClick.x >= 500 && relClick.x <= 564 && relClick.y >= 350 && relClick.y <= 414) {
            ...
            public boolean touchDragged(int screenX, int screenY, int pointer) {
                if (debugManager.isDebugMode() && debugManager.hasDragTarget()) {
                    debugManager.handleDebugDrag(screenX, screenY, camera, viewport);
                    return true;
                }
                return false;
            }
            ...
            public boolean keyDown(int keycode) {
                ...
                if (keycode == Input.Keys.F2 && debugManager.isDebugMode()) { debugManager.exportDebugCoordinates(); return true; }
    ...
    public void render(float delta) {
        ...
        debugManager.render(batch, camera, viewport);
        game.getStage().draw();
